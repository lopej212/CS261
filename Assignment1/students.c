 ...
 * address below:
 *
 * Name: Jose Manuel Lopez Alcala
 * Email: lopezalj@oregongstate.edu
 */
...
void init_student(struct student* student, char* name, int id, float gpa) {
  ...
  // assert(name_copy != 0);
   strcpy(name_copy, name);//This copies name parameter to name_copy 
...
struct student* deep_copy_student(struct student* student) {
   ...
   struct student *student_copy = (struct student*) malloc(sizeof(struct student));
//   assert( student_copy != 0);

   student_copy->name = (char*)malloc( (strlen(student->name)+1 ) * sizeof(char));
...
struct student* create_student_array(int num_students, char** names, int* ids,
   ...
   struct student *array_students = (struct student*) malloc(num_students * sizeof(struct student));
   
   for(int i = 0; i < num_students; i++)
   {

      init_student(&array_students[i], names[i], ids[i], gpas[i]);
   }
...
struct student* find_max_gpa(struct student* students, int num_students) {
   ...
   for(int i=0; i < num_students; i++)
   {
      if( students[i+1].gpa > students[index].gpa)
      {
	 index = (i+1); 
      }
   }
...
struct student* find_min_gpa(struct student* students, int num_students) {
   ...
   {
      if(students[index_low].gpa >  students[k].gpa)
      {
	 index_low = (k);
      }
   }
   return &students[index_low];
}
...
void sort_by_gpa(struct student* students, int num_students) {
   ...
   for(int i= 0; i < num_students; i++)
   {
      for(int q = i+1; q < num_students;q++)
      {
	 if( students[q].gpa > students[i].gpa)
	 {
	    struct student temp = students[i];
	    students[i] = students[q];
	    students[q] = temp;
	 }
      }
   }
